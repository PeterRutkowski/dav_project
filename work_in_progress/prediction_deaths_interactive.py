...
Y = list(list(df['Number of new daily deaths'])+list(pred))
...
fig.update_layout(
    title='Predicted evolution of new daily COVID-19 deaths\n'
             'in Switzerland using ARMA model',
    autosize=False,
    width=680,
    height=500,
    xaxis=dict(title='Date', tickangle=45, tickfont=dict(size=10), titlefont=dict(size=10)),
    yaxis=dict(title='New daily deaths', tickfont=dict(size=10), titlefont=dict(size=10)),
    titlefont=dict(size=12),
    title_x=0.5
)

fig.write_html('plotly_prediction_deaths.html', include_plotlyjs='cdn')